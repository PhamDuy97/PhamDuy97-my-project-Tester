...
public class CreateKhongNhapDuLieu {
	...
	@Test
	public void testAddUserFunctionalityWithEmptyData() {
		...
		// Nhập ID
		String idValue = "";
		webDriver.findElement(By.id("id")).sendKeys(idValue);

		// Nhập tên ngành
		String nameValue = "";
		webDriver.findElement(By.id("name")).sendKeys(nameValue);

		// Nhập viết tắt
		String abbreviationValue = "";
		webDriver.findElement(By.id("abbreviation")).sendKeys(abbreviationValue);
		...
		WebElement idErrorElement = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//label[@id='id-error' and contains(text(),'Bạn chưa nhập mã ngành')]")));
		WebElement nameErrorElement = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//label[@id='name-error' and contains(text(),'Bạn chưa nhập tên ngành')]")));
		WebElement abbreviationErrorElement = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//label[@id='abbreviation-error' and contains(text(),'Bạn chưa nhập tên viết tắt của ngành')]")));
		//        WebElement programTypeErrorElement = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//label[@id='program_type-error' and contains(text(),'Bạn chưa chọn CTĐT')]")));

		// Kiểm tra xem các thông báo lỗi đã được hiển thị hay không
		Assert.assertTrue(idErrorElement.isDisplayed(), "ID error message is not displayed when submitting empty data.");
		Assert.assertTrue(nameErrorElement.isDisplayed(), "Name error message is not displayed when submitting empty data.");
		Assert.assertTrue(abbreviationErrorElement.isDisplayed(), "Abbreviation error message is not displayed when submitting empty data.");
		//        Assert.assertTrue(programTypeErrorElement.isDisplayed(), "Program type error message is not displayed when submitting empty data.");
	}
